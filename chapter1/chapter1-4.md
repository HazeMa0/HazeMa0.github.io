...
>
> 事实上，先驱们准备的这些基础设施至今仍在发挥作用——C语言程序先被编译为汇编语言，然后才转化为二进制指令。你可以使用`gcc -S test.c`将test.c转化为汇编语言程序。
> 
...
```c
for (刚进入for循环要做的事情; 满足这个条件就不退出for循环; 执行一次循环后要做的事情)
{
    // 要循环执行的代码
}
```
...
> **练习题（选自南大程序设计基础OJ）**
>
> 1. 给定一个大于1的正整数n，请你设计一个程序输出小于等于n的所有素数。
>
> 2. 某年某月，没有日历的小蓝鲸想知道这个月有多少天，请你帮小蓝鲸计算该月的天数。
>
> 3. 输入一个正整数m，重复以下操作：如果m < 10，输出m，结束程序；否则，令m等于m中各位非零数字的乘积。
>
> 请在main函数中进行上述操作，并输出m。
>
> 4. 小蓝鲸最近沉迷一种叫《环》的游戏。这个游戏的地图是一个圆环，上面有n个格子。规则如下：
>
