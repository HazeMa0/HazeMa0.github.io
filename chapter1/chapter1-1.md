...
就像打开网页需要浏览器、聊天需要使用微信与QQ一样，要编写程序，我们也要做好相应的准备。接下来，我们将一起完成编程所需软件的安装，顺便学点有趣的东西。我们以`Windows`系统安装`Visual Studio Code (VSCode)`为例进行开发环境的配置。

> [!question]
> **什么是Windows操作系统？我使用的是什么操作系统？**
> 
> A：<h1><b>这个问题你应该去问问搜索引擎（例如百度或谷歌）。</b></h1>
> 
...
> **可选任务01：自己动手**
> 
> 你可以通过实践让自己更快获得独立解决问题的能力。不阅读本小节的内容，通过搜索引擎搜索`系统具体版本 VSCode C语言环境`尝试自己完成安装。例如：`Windows11 VSCode C语言环境`（**注：**在搜索引擎中如何搜索多个关键词呢？方法就是在这几个词语之间加上空格，这样搜索引擎会向你提供与这几个词语有关的内容）。
> 
...
1. 下方注明`广告`二字的**一定不**是官方链接；
2. `XX软件园` `XX下载站`**一定不**是官方链接；
3. 注明蓝色`官方`二字的**一般**是官方链接；
...
> [!question]
> **文件有各种各样的类型，但本质来说都是一堆0和1组成的。系统为什么可以分辨这个是Word文档，那个是安装包呢？**
> 
...
>
> Q：我猜不出来，exe到底代表什么？有什么别的后缀名也代表可执行文件吗？
>
> A：去问问搜索引擎吧。
...
> [!question]
> **我们安装软件一般可以选择安装位置。听说软件是不能装到C盘的，这是为什么呢？还有什么是C盘，它为什么这么特殊？**
>
...
> 
> 但是，时代变了，随着硬盘性能的提升，通过分区提高读取效率的意义已经消失，现在对硬盘进行分区单纯是为了方便文件管理（想一想，如果要找一个文件，直接在专门的分区找方便还是从C盘的一大堆英文文件夹里翻找方便？）。如果你的电脑根本没有进行分区，那自然只能安装在C盘，至于分不分区我只能说你开心就好（想要进行分区？我后续可能会出教程，但是在此之前去网上找找办法是个好主意）。
>
> 最后，我们来介绍如何写一个文件或者文件夹的**路径**。比如D盘（`D:\`）里有一个叫做`文档`的文件夹，这个文件夹里面有一个`task1.pptx`的文件。它的路径写作：
> 
> `D:\文档\task1.pptx`
>
...
> [!question]
> **安装程序提到了“快捷方式”这个词语。这是什么意思？**
>
> A：文件和程序被存放在各级文件夹中。怎么更快打开文件和程序呢？Windows提供了一种特殊的文件，叫做“快捷方式”（后缀名.lnk，默认不显示）作为快速打开软件的虫洞。我必须要提醒的一件事是：桌面上的和开始菜单里的程序都是左下角带箭头的快捷方式，删除了“虫洞”并不会影响软件本身。
>
> Q：那又怎样呢？
>
> A：那意味着桌面上留你需要经常打开的软件就好，其他的可以通通删掉，以后要用就在开始菜单里面找；这意味着只需要一些简单的百度你就能掌握新建快捷方式的方法，这样你也可以更快地访问文件和程序：这意味着复制文件的时候小心只复制了快捷方式！
>
...
这里我们直接给出`MinGW-W64`（内含gcc）的下载路径（有兴趣的朋友也可以挑战自己去找链接）：
...
> [!question]
> **来自笔者的反问：根据之前对安装位置的介绍，你认为应该把gcc解压在哪里更合适？（单选）**
>
> A. 随意位置无所谓
>
> B. 已经分区的电脑，解压到C盘
...
> C. D:\gcc
>
> D. D:\hbdkghngicnhgnvu
>
> E. D:\编译器
>
> F. E:\Documents\code\gcc
>
> 很遗憾，上面这些选项你可能会觉得有些搞笑，但是无数人真的会这么干。现在你应该知道要解压在哪里了吧。

### 第三步 大道至简——使用终端

影视作品里的黑客总是对着一个黑乎乎的界面敲打神秘的代码，接下来我们就要进入这个神秘的界面一探究竟了。

如果你使用Windows 10/11系统，请先安装`终端`。如果你对安装额外软件的意义感到怀疑，不妨去B站搜索一下这个软件的风评然后自己决定。在开始菜单里打开`Microsoft Store`，搜索`终端`，安装第一个名叫`Windows Terminal(终端)`的软件。
...
真是个好消息——那我们可以通过修改PATH来用终端启动QQ了！先别急，启动QQ没啥意思，启动`GCC`才是我告诉你这么多东西的真正原因（坏笑）。还记得你之前复制的gcc文件夹的bin目录的路径吗？只要把这个路径输入`PATH`，你就可以只输入gcc来启动gcc了，而不需要给出路径，这实在是太方便了！
...
你要做的第一步是在文件管理器里面新建一个文件夹，项目的文件夹名称也应当全部由英文构成（例子：`D:\learn-c`）。

> [!question]
> **来自笔者的又一次反问：看看笑话**
>
> A. D:\StudyHardEveryDay    （无意义的文件夹名称）
>
> B. D:\新建文件夹(1)(1)(1)   （求你改个名字吧）
>
> C. D:\gcc\code             （gcc程序和你写的代码不应该混在一起）
>
> D. D:\                     （直接放在根目录你不嫌乱吗，想象一下半学期后你的D盘同时有几百个C语言程序......）
>
> E. C:\Users\Admin\Documents   （所有代码都存在默认位置是吧）
>
> F. 现实只会比上面的例子更魔幻......

然后打开VSCode，在上面的`文件`菜单中找到`打开文件夹`，找到你建好的文件夹，这样你就打开了用于开发的文件夹。

在侧栏找到新建文件的按钮，新建一个文件，起名为`hello.c`（.c就是C语言文件）。

> [!question]
> **来自笔者的双一次反问：在半学期后，你最好可以根据文件名称快速找到今天写的文件。这就是一个好的名字的意义。**
>

![图片显示失败](pic_1_1_06.png)

在`hello.c`里面写入如下代码（不用理解代码内容）：
...
这个时候，你可以去VSCode自带的终端查看程序的运行结果。在VSCode自带的终端按下回车，结束程序的运行。
