...
> 
> 请在百度或谷歌搜索`Windows`、`macOS`，随意地略读一下搜索引擎给出的介绍和搜索引擎给出的图片（这时候你应该搜索`Windows 图片`），这样你通过对比应该就会知道自己的电脑是什么系统了。
...
于是，你找到了 https://code.visualstudio.com 这个官方网站（你真的找到了吗？），可是网站全是洋文让你有点头晕。但是，那个大大的`Download For Windows`你肯定能看懂，点击那个按钮即可开始下载。
...
如果你使用`Windows`系统，**在安装前**，请先阅读下面的Q&A板块。请安装到一个**英文路径**，如果有多个分区请不要放到C盘。
...
>
> 最后，我们来介绍如何写一个文件或者文件夹的路径。比如D盘里文档文件夹里有一个叫PPT幻灯片的文件夹，这个文件夹里面有一个task1.pptx的文件。它的路径写作：
> 
> D:\PPT幻灯片\task1.pptx
>
> 其中，冒号和\（我们称作反斜杠，猜猜斜杠长啥样子？）都要在英文输入法里输入。尝试一下输入英文输入法下的冒号和中文输入法下的冒号，对比一下他们细微的区别。
>
> 还有一件事，由于一些原因，软件的安装的路径应当全部是英文和数字。
> 
